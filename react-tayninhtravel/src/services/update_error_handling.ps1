...
foreach ($service in $services) {
    ...
    if (Test-Path $service) {
        ...
        $content = $content -replace "catch\s*\(error[^)]*\)\s*\{\s*console\.error[^}]+throw\s+error;\s*\}", @"
catch (error: any) {
...
"@
        ...
        # Pattern 2: catch (error) { console.error(...); return null/[]; }
        $content = $content -replace "catch\s*\(error[^)]*\)\s*\{\s*console\.error[^}]+return\s+(null|\[\]);?\s*\}", @"
catch (error: any) {
        // Error already shown by axios interceptor
...
Write-Host "All services have been updated!" -ForegroundColor Cyan
