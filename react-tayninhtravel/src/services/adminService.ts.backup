...
class AdminService {
  ...
  async approveTour(tourId: string, comment: string = ""): Promise<any> {
    ...
    const response = await axios.post(
      `/Cms/tourdetails/${tourId}/approve-reject`,
      {
        isApproved: true,
        comment,
      }
    ...
    try {
      ...
      return {
        blogs: response.data.data
          ? response.data.data.map(this.mapApiBlogToAdmin)
          : [],
  ...
  async getCmsBlogs(
    ...
    try {
      const { pageIndex = 0, pageSize = 10, textSearch = "" } = params;
  ...
  private mapApiBlogToAdmin(apiBlog: any): AdminBlogPost {
    return {
      ...
      tags: apiBlog.tags || [],
      featuredImage: Array.isArray(apiBlog.imageUrl)
        ? apiBlog.imageUrl[0]
        : apiBlog.imageUrl || apiBlog.featuredImage,
  ...
  async updateTicketStatus(
    ...
    try {
      ...
      await axios.put(`SupportTickets/Admin/${ticketId}/status`, {
        status: statusNumber,
      });
    } catch (error) {
  ...
  async respondToTicket(ticketId: string, response: string): Promise<void> {
    try {
      ...
      await axios.post(`Cms/${ticketId}/Reply-SupportTicket`, {
  ...
  async approveTourGuideApplication(cvId: string): Promise<any> {
    const response = await axios.put(
      `Cms/tourguide-applications/${cvId}/approve`
    );
    return response.data;
  }
  async rejectTourGuideApplication(cvId: string, reason: string): Promise<any> {
    const response = await axios.put(
      `Cms/tourguide-applications/${cvId}/reject`,
      { reason }
    );
    return response.data;
  }
  ...
  async getShopRegistrations({
    ...
    if (response.data) {
      ...
      if (
        response.data.data &&
        Array.isArray(response.data.data.applications)
      ) {
        return response.data;
      }
  ...
  async approveShopRegistration(applicationId: string): Promise<any> {
    const response = await axios.post(
      `SpecialtyShopApplication/${applicationId}/approve`
    );
    return response.data;
  }

  // Từ chối đơn đăng ký shop
  async rejectShopRegistration(
    applicationId: string,
    reason: string
  ): Promise<any> {
    const response = await axios.post(
      `SpecialtyShopApplication/${applicationId}/reject`,
  ...
  async getTourGuides(
    ...
    try {
      ...
      const response = await axios.get("/Cms/TourGuide", {
        params: queryParams,
      });
      ...
      return {
        ...
        message:
          ...
          "Không thể lấy danh sách hướng dẫn viên",
      };
    }
  }

  /**
   * Lấy thống kê doanh thu của các tour companies
   */
  async getRevenueStats(
    ...
    try {
      const response = await axios.get("/Admin/tour-companies/revenue-stats", {
        params,
      });
      return response.data;
    } catch (error) {
  ...
  /**
   * Lấy chi tiết doanh thu của một tour company
   */
  async getRevenueDetail(
    ...
    try {
      const response = await axios.get(
        `/Admin/tour-companies/${tourCompanyId}/revenue-detail`,
        { params }
      );
      return response.data;
    } catch (error) {
  ...
  /**
   * Lấy thông tin tour slot theo ID
   */
  async getTourSlotById(tourSlotId: string): Promise<any> {
    try {
  ...
  /**
   * Cập nhật trạng thái tour slot
   */
  async updateTourSlotStatus(tourSlotId: string, status: string): Promise<any> {
    try {
      const response = await axios.put(
        `/Admin/tour-slots/${tourSlotId}/status`,
        { status }
      );
      return response.data;
    } catch (error) {
