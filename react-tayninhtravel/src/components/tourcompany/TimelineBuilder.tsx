...
import { SpecialtyShop, TimelineItem } from "../../types/tour";
...
const TimelineBuilder: React.FC<TimelineBuilderProps> = ({
  ...
  const getDefaultTimelineItems = (): TimelineFormItem[] => [
    {
      checkInTime: "06:00",
      activity: "Check-in",
      location: "Điểm tập trung",
      sortOrder: 1,
      isNew: true,
    },
    {
  ...
  useEffect(() => {
    if (timeline && timeline.length > 0) {
      ...
      const initialItems: TimelineFormItem[] = timeline
        .sort(
          (a, b) =>
            (a.sortOrder || a.orderIndex || 0) -
            (b.sortOrder || b.orderIndex || 0)
        )
        .map((item) => ({
          id: item.id,
      ...
      // Auto-generate default timeline items for new timeline
      const defaultItems = getDefaultTimelineItems();
    ...
    if (onUpdate) {
      const updatedTimeline = reorderedItems
        ...
        .map((item) => ({
          ...
          specialtyShop:
            specialtyShops.find((shop) => shop.id === item.specialtyShopId) ||
            null,
  ...
  const saveTimeline = async () => {
    ...
    try {
      ...
      if (newItems.length > 0) {
        ...
        const response = await createTimelineItems(
          createRequest,
          token ?? undefined
        );
        if (response.success) {
          message.success(
            `Tạo ${
              response.data?.createdCount || newItems.length
            } timeline items thành công`
          );
        }
      }
              ...
              <Row gutter={16}>
                <Col span={6}>
                  <Input
                    ...
                    onChange={(e) =>
                      updateItem(index, "checkInTime", e.target.value)
                    }
                ...
                <Col span={6}>
                  <Select
                    ...
                    onChange={(value) =>
                      updateItem(index, "specialtyShopId", value)
                    }
                    allowClear
                    style={{ width: "100%" }}>
                ...
                <Col span={2}>
                  <InputNumber
                    ...
                    onChange={(value) =>
                      updateItem(index, "sortOrder", value || 1)
                    }
                    style={{ width: "100%" }}
                  />
                </Col>
