...
const TourPriceDisplay: React.FC<TourPriceDisplayProps> = ({
    ...
    return (
        <div className={`tour-price-display ${className} tour-price-size-${size}`}>
            <Space direction="vertical" size={2}>
                ...
                <Space align="baseline" size={8}>
                    {pricingInfo.isEarlyBird ? (
                        <>
                            {/* Discounted Price */}
                            <Text strong className="tour-price-final">
                                {formatPriceNumber(pricingInfo.finalPrice)}₫
                            </Text>
                            {/* Original Price (crossed out) */}
                            <Text delete type="secondary" className="tour-price-original">
                    ...
                    <Space size={4} wrap>
                        ...
                        {pricingInfo.isEarlyBird && (
                            <Tooltip title={t('tours.earlyBirdTooltip', `Giảm ${pricingInfo.discountPercent}% cho khách đặt sớm`, { percent: pricingInfo.discountPercent })}>
                                <Tag color="red" icon={<FireOutlined />} className="tour-price-tag tour-price-tag-earlybird">
                        ...
                        {pricingInfo.isEarlyBird && remainingDays > 0 && (
                            <Tooltip title={t('tours.earlyBirdCountdownTooltip', 'Số ngày còn lại để được giảm giá Early Bird')}>
                                <Tag color={isEndingSoon ? 'orange' : 'blue'} icon={<ClockCircleOutlined />} className="tour-price-tag tour-price-tag-countdown">
                        ...
                        {/* Last Minute Tag */}
                        {!pricingInfo.isEarlyBird && (
                            <Tag color="default" className="tour-price-tag tour-price-tag-regular">
